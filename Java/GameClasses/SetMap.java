...
public class SetMap implements Initializable
{
        ...
        for(Tile tile : tiles)
        {
            ...
            tile.setOnMouseClicked(event ->
            {
                if((tile.getTileNumber() + 1) % NUM_PER_ROW != 0 && tile.getTileNumber() < NUM_OF_TILES - NUM_PER_ROW &&
                        tile.getTileNumber() % NUM_PER_ROW != 0 && tile.getTileNumber() > NUM_PER_ROW - 1)
                {
                    if(commandIF(tileNumber))
                    {
    ...
    private boolean commandIF(int tileNumber)
    {
        if(!(tiles.get(tileNumber).isChecked() || tiles.get(tileNumber + x).isChecked() ||
