...
public class StartGame implements Initializable
{
    ...
    private static List<MediumTiles> tiles = new ArrayList<>();
    private static List<SmallTiles> myTiles = new ArrayList<>();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle)
    {
        ...
        ThreadSave.setSoldiers(soldiers);

        addHorseman();

        ThreadSave.setHorsemen(horsemen);

        addCommandCenter();

        ThreadSave.setCommandCenter(commandCenter);

        addCastles();

        ThreadSave.setCastles(castles);
        ...
        nextTurnButton.setOnAction(actionEvent -> {
            ...
            if(commandCenter.isCanAttack())
            {
                count++;
            }
            if(ThreadSave.isIsPlayersTurn() && count == 0)
            {
                nextTurn();
            }
        });
    ...
    public static void nextTurn()
    {
        ...
        ThreadSave.setJesus(false);

        if (ThreadSave.isIsPlayersTurn())
        {
            count = 0;
            try
            {
                ...
                //send number of checked tiles to server
        ...
        //send number of hit tiles to server to check for winner
        for(SmallTiles tile : myTiles)
        {
            if(tile.isChecked() && tile.isAttacked())
            {
                hitCount++;
            }
        }
        ...
        for(MediumTiles tile : tiles)
        {
            tile.setOnMouseClicked(mouseEvent -> {
                ...
                if(tileNumber < SetMap.getNumOfTiles() - SetMap.getNumPerRow() && castles.get(position).isCanAttack()
                        && ((tile.getTileNumber() + 1 ) % SetMap.NUM_PER_ROW != 0) && ThreadSave.isIsPlayersTurn())
                {
            ...
            tile.setOnMouseClicked(mouseEvent -> {
                ...
                if(isVertical && horsemen.get(position).canAttack())
                {
                    if(tileNumber < SetMap.getNumOfTiles() - SetMap.getNumPerRow() && !tile.isChecked() &&
                            !tiles.get(tileNumber + 1).isChecked() && horsemen.get(position).canAttack() && ThreadSave.isIsPlayersTurn())
                    {
                        tile.setChecked(true);
                        tiles.get(tileNumber + x).setChecked(true);
                ...
                else if(!isVertical && horsemen.get(position).isCanAttack())
                {
    ...
    private void soldierAction(int position)
    {
        for (MediumTiles tile : tiles)
        {
            ...
            {
                if (soldiers.get(position).isCanAttack() && ThreadSave.isIsPlayersTurn() && !tile.isChecked())
                {
                    tile.setChecked(true);
                    tile.getBorderOfTile().setFill(Paint.valueOf("blue"));
